...
def get_airplane_shape(flight, debug_enabled=False):
    ...
    db_entry = DB.get(type_designator, {})
    type_description = db_entry.get("desc")
    wtc = db_entry.get("wtc")
    ...
    if type_description:
        ...
        # Try plain type_description
        if type_description in TYPE_DESCRIPTION_ICONS:
            return SHAPES[TYPE_DESCRIPTION_ICONS[type_description]]
        
        # Try first character of type_description
        basic_type = type_description[0] if type_description else ""
